...
public:
  ...
  value_type get_value(unsigned int q) const
  {
    ...
    value_type u_cg = phi_cg.get_value(q); // The Continuous Galerkin component
  ...
  gradient_type get_gradient(unsigned int q) const
  {
    ...
    // CG properties
    value_type u_cg      = phi_cg.get_value(q);
    gradient_type grad_u_cg = phi_cg.get_gradient(q);
  ...
  get_divergence(unsigned int q) const
  {
    ...
    return phi_dg.get_divergence(q) + (psi * phi_cg.get_divergence(q)) + (phi_cg.get_value(q) * nabla_psi);
  }

  value_type get_normal_derivative(unsigned int q) const
  {
    value_type du_dn_dg = phi_dg.get_normal_derivative(q);
    value_type du_dn_cg = phi_cg.get_normal_derivative(q);
    value_type u_cg     = phi_cg.get_value(q);
  ...
  void submit_gradient(gradient_type const & val, unsigned int q) 
  {
    ...
    phi_cg.submit_gradient(psi * val, q);

    phi_cg.submit_value(val * nabla_psi, q); 
  }
  ...
  submit_divergence(scalar const & val, unsigned int q)
  {
    ...
    phi_cg.submit_divergence(psi * val, q);
  ...
  void submit_normal_derivative(value_type const & val, unsigned int q)
  {
    ...
    phi_cg.submit_normal_derivative(psi * val, q);

    phi_cg.submit_value(dpsi_dn_face * val, q);
  }
                         ...
                         std::pair<VectorType*, VectorType*> & dst)
  {
    ...
    phi_cg.integrate_scatter(flags, *(dst.second));
  }
