...
CombinedOperator<dim, n_components, Number>::CombinedOperator() : scaling_factor_mass(1.0)
{
}

template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::initialize(
...
CombinedOperator<dim, n_components, Number>::initialize(
  ...
  dealii::AffineConstraints<Number> const &                 affine_constraints,
  CombinedOperatorData<dim> const &                         data,
  std::shared_ptr<Operators::ConvectiveKernel<dim, n_components, Number>> convective_kernel_in,
  std::shared_ptr<Operators::DiffusiveKernel<dim, n_components, Number>>  diffusive_kernel_in)
{
  operator_data = data;
...
CombinedOperator<dim, n_components, Number>::get_data() const
{
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::update_after_grid_motion()
{
  if(operator_data.diffusive_problem)
...
CombinedOperator<dim, n_components, Number>::get_velocity() const
{
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::set_velocity_copy(VectorType const & velocity) const
{
  if(operator_data.convective_problem)
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::set_velocity_ptr(VectorType const & velocity) const
{
  if(operator_data.convective_problem)
...
template<int dim, int n_components, typename Number>
Number
CombinedOperator<dim, n_components, Number>::get_scaling_factor_mass_operator() const
{
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::set_scaling_factor_mass_operator(Number const & scaling_factor)
{
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::reinit_cell_derived(IntegratorCell &   integrator,
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::reinit_face_derived(IntegratorFace &   integrator_m,
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::reinit_boundary_face_derived(IntegratorFace &   integrator_m,
                                                            unsigned int const face) const
{
  if(operator_data.convective_problem)
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::reinit_face_cell_based_derived(
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_cell_integral(IntegratorCell & integrator) const
{
  for(unsigned int q = 0; q < integrator.n_q_points; ++q)
  {
    simd_value_type value = integrator.get_value(q);
    gradient_type gradient = integrator.get_gradient(q);

    gradient_type gradient_flux = gradient * 0.0;
    simd_value_type value_flux = value * 0.0;
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_face_integral(IntegratorFace & integrator_m,
                                                IntegratorFace & integrator_p) const
{
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    simd_value_type value_m = integrator_m.get_value(q);
    simd_value_type value_p = integrator_p.get_value(q);

    vector normal_m = integrator_m.get_normal_vector(q);

    simd_value_type value_flux_m = value_m * 0.0;
    simd_value_type value_flux_p = value_p * 0.0;

    if(operator_data.convective_problem)
    {
      std::tuple<simd_value_type, simd_value_type> flux = convective_kernel->calculate_flux_interior_and_neighbor(
    ...
    if(operator_data.diffusive_problem)
    {
      simd_value_type normal_gradient_m = integrator_m.get_normal_derivative(q);
      simd_value_type normal_gradient_p = integrator_p.get_normal_derivative(q);
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_face_int_integral(IntegratorFace & integrator_m,
                                                    IntegratorFace & integrator_p) const
{
  (void)integrator_p;
  ...
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    // set value_p to zero
    simd_value_type value_m = integrator_m.get_value(q);
    simd_value_type value_p = value_m * 0.0;

    vector normal_m = integrator_m.get_normal_vector(q);

    simd_value_type value_flux = value_m * 0.0;
    ...
    if(operator_data.diffusive_problem)
    {
      ...
      simd_value_type normal_gradient_m = integrator_m.get_normal_derivative(q);
      simd_value_type normal_gradient_p = normal_gradient_m * 0.0;
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_face_int_integral_cell_based(IntegratorFace & integrator_m,
                                                               IntegratorFace & integrator_p) const
{
  (void)integrator_p;
  ...
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    ...
    simd_value_type value_flux = value_p;
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_face_ext_integral(IntegratorFace & integrator_m,
                                                    IntegratorFace & integrator_p) const
{
  (void)integrator_m;
  ...
  for(unsigned int q = 0; q < integrator_p.n_q_points; ++q)
  {
    // set value_m to zero
    simd_value_type value_p = integrator_p.get_value(q);
    simd_value_type value_m = value_p * 0.0;

    // n⁺ = -n⁻
    vector normal_p = -integrator_p.get_normal_vector(q);

    simd_value_type value_flux = value_m;
    ...
    if(operator_data.diffusive_problem)
    {
      ...
      simd_value_type normal_gradient_m = normal_gradient_p * 0.0;
...
template<int dim, int n_components, typename Number>
void
CombinedOperator<dim, n_components, Number>::do_boundary_integral(
  IntegratorFace &                   integrator_m,
  ...
  dealii::types::boundary_id const & boundary_id) const
{
  ...
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    simd_value_type value_m = calculate_interior_value(q, integrator_m, operator_type);
    simd_value_type value_p = calculate_exterior_value(value_m,
                                              q,
                                              integrator_m,
                                              operator_type,
                                              boundary_type,
    ...
    if(operator_data.diffusive_problem)
    {
      simd_value_type normal_gradient_m = calculate_interior_normal_gradient(q, integrator_m, operator_type);
      simd_value_type normal_gradient_p = calculate_exterior_normal_gradient(normal_gradient_m,
                                                                    q,
                                                                    integrator_m,
                                                                    operator_type,
                                                                    boundary_type,
...
template class CombinedOperator<2, 1, float>;
template class CombinedOperator<2, 1, double>;

template class CombinedOperator<3, 1, float>;
template class CombinedOperator<3, 1, double>;

template class CombinedOperator<2, 2, float>;
template class CombinedOperator<2, 2, double>;

template class CombinedOperator<3, 2, float>;
template class CombinedOperator<3, 2, double>;
